...
relabelling of HS sectors (ARI/NMI even higher); and out-of-block exploration still tracks destination breadth,
not export volume (correlation with reach $0.16$ against $0.06$ with value). The one result that softens is the
flow mechanism --- at $\gamma=2$ in-block diversification no longer grows cleanly faster than out-of-block in
proportion, though the share still falls with volume. The reading is reassuring: $\gamma=1$ is a sensible
reporting default, giving five economically interpretable blocks without over-splitting coherent sectors, and
the findings do not depend on that choice.
