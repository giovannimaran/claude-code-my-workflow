...
% ---- title block ------------------------------------------------------------
\title{\vspace{-1.0cm}\textbf{\large Deepen or Diversify?}\\[4pt]
\Large Estimating Firm Capability Reconfiguration under Trade Shocks}
...
next step (\S\ref{sec:panel}). Sections~\ref{sec:question}--\ref{sec:panel} lay out the
design and its problems; the open questions for discussion are collected in
\S\ref{sec:questions}.
\end{keybox}
...
% =============================================================================
\section{From description to causation: the design and its problems}
\label{sec:panel}
% =============================================================================
...
is genuinely open. I lay out the problem and my candidate fixes here; the open questions
they raise are collected in \S\ref{sec:questions}.
...
spirit of a clean control group. The open choice is whether the cleaner estimand is worth a
held-fixed (and therefore stale) map, or whether the capability geometry can be made to move
\emph{with} the data without moving \emph{with the treatment}. Concretely it is a question
about the \emph{detection algorithm} itself: whether BRIM (or any community-detection
method) can deliver a partition that updates with the data yet stays, by construction, blind
to the shock. I take this back up in \S\ref{sec:questions}.
...
\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{The held-fixed-vs-evolving map trade-off} (the open seam of
        ...
        altogether, at the cost of losing the crisp in/out split? And on the method itself:
        can a community-detection partition be made to evolve with the data yet stay blind
        to the shock, or is that the wrong way to frame it? Which would you reach for?
...
\begin{enumerate}[leftmargin=1.5em]
  \item If you wanted to rationalise an in-block/out-of-block margin, \emph{what backbone
        would you reach for}? My own instinct drifts toward a multi-product firm in the
        ``core competence'' tradition (Eckel--Neary 2010; Mayer--Melitz--Ottaviano 2014),
        where products sit on a ladder ranked by distance from the firm's core competence
        and tougher competition skews the mix \emph{toward} the core --- which is exactly
        my retrenchment prediction, with in-block $\approx$ near-core and out-of-block
        $\approx$ far-from-core. The honest gap is that their ladder is ordered by
        \emph{cost/productivity}, whereas my capability blocks are ordered by
        \emph{relatedness} (network co-occurrence): is ``distance from core'' the same
        object in the two senses, or do I need a genuine capability/relatedness model
        instead? That is the piece I would most like your reaction to.
  ...
  \item Does any of this \emph{need} general equilibrium? A sector-wide tariff shock is an
        aggregate phenomenon, so its firm-level effects might be better captured inside a
        quantitative trade model --- which would also let me speak to the \emph{welfare} of
        retrenchment, not just its sign. Is that worth the modelling cost, or a distraction
        from a clean reduced-form paper?
...
choice and the continuous dose), by \emph{measurement} (intermediaries and the
balanced-sample selection), or
...
\begin{itemize}[leftmargin=1.4em,labelsep=0.4em]\raggedright
  ...
  \item Eckel, C., Neary, J. P. (2010). Multi-product firms and flexible manufacturing in the global economy. \emph{Review of Economic Studies} 77(1): 188--217.
  ...
  \item Mayer, T., Melitz, M. J., Ottaviano, G. I. P. (2014). Market size, competition, and the product mix of exporters. \emph{American Economic Review} 104(2): 495--536.
